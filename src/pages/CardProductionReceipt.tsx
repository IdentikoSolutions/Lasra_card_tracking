...
import { GridBox, OverlayCard } from '../styles/styles'
  ...
  return (
    <div >
      ...
      <div id='content'>
        ...
        {!active&&<ButtonElement label={active ? "close report" : "open report"} onClick={() => toggle(!active)} className='mb-4'/>}
       <br/>
        <DetailContainer title={'Batch Details: '}>
          ...
          <DetailField type='date' label="Date created:" bg={color.auto} value={batchDetail?.bankDataCreatedOn?.substring(0, 10)} />
        </DetailContainer>
        <InputFieldContainer title={'Card received details: '}>
          <InputField type='date' label="Date received:" value={receiptDetail.date} onChange={(e) => setReceiptDetail({ ...receiptDetail, date: e.target.value })} bg={color.action} />
        ...
        {cards.length > 0 &&
          active &&
          <OverlayCard>
              {active &&    <ButtonElement label={active ? "close report" : "open report"} onClick={() => toggle(!active)} />}
